...
namespace VATClient
{
    public partial class FormSaleImportDHL : Form
    {
        #region Variables
        ...
        private string FileLocationIBS = "";
        private string FileLocationIAS = "";
        private string FileLocationPDF = "";


        #endregion

        public FormSaleImportDHL()
        {
            InitializeComponent();
        ...
        private void FormMasterImport_Load(object sender, EventArgs e)
        {
            ...
            FileLocationIBS = dal.settingsDesktop("Integration", "FileLocationIBS", null, connVM);
            FileLocationIAS = dal.settingsDesktop("Integration", "FileLocationIAS", null, connVM);
            FileLocationPDF = dal.settingsDesktop("Integration", "FileLocationPDF", null, connVM);

            if (string.IsNullOrWhiteSpace(FileLocationPDF) || FileLocationPDF == "-")
            {
                FileLocationPDF = "D:\\invoice";
            }
        ...
        private void btnBulk_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    ibsPath = FileLocationIBS;
                    iasPath = FileLocationIAS;
            ...
            try
            {
                if (!isProcessRunning)
                {
                    FileLogger.Log("From Sale Import DHL", "timer1_Tick",
                        "Entered : " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));

                    int iasFiles = Directory.EnumerateFiles(iasPath).Count();
                    int ibsFiles = Directory.EnumerateFiles(ibsPath).Count();
                    ...
                    salesData = importDal.GetSaleDHLAirportDbData("", dtBranchInfo);


                    //&& fileProcessDateTime < Convert.ToDateTime(DateTime.Now.ToString("HH:mm:ss"))
                    //&& fileProcessDateTime < Convert.ToDateTime(DateTime.Now.ToString("HH:mm:ss"))

                    if (salesData.Rows.Count > 0)
                    {
                        FileLogger.Log("From Sale Import DHL", "timer1_Tick",
                            "CRAT : " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));

                        progressBar1.Visible = true;
                        isProcessRunning = true;
                        OrdinaryVATDesktop.IsDHLCrat = true;
                        bgwAirport.RunWorkerAsync();
                    }
                    else if (iasFiles > 0)
                    {
                        progressBar1.Visible = true;
                        isProcessRunning = true;
                        OrdinaryVATDesktop.IsDHLCrat = false;
                        FileLogger.Log("From Sale Import DHL", "timer1_Tick", "ias : " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));

                        bgwIAS.RunWorkerAsync();
                    ...
                    else if (ibsFiles > 0)
                    {
                        ...
                        progressBar1.Visible = true;
                        isProcessRunning = true;
                        FileLogger.Log("From Sale Import DHL", "timer1_Tick", "ibs : " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));

                        bgwDHL.RunWorkerAsync();
                    }
                    else
                    {
                        FileLogger.Log("From Sale Import DHL", "timer1_Tick", "report : " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private void bgwReportGeneration_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                RptVAT6_7_V12V2 = new RptVAT6_7_DHL_V12V2();
                ...
                RptVAT6_8_V12V2 = new RptVAT6_8_DHL_V12V2();
            ...
            RptVAT6_3_DHL2_V12V2 objrpt, FormulaFieldDefinitions crFormulaF, string pathRoot, ImportDAL dal)
        {
            ...
            for (int i = 0; i < iasCounter; i++)
            {
                ...
                MoveToFolder(fileDirectory, FileLocationPDF);
