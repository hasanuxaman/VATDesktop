    ...
    public partial class FormTracking : Form
    {
        ...
        private static List<TrackingVM> trackings = new List<TrackingVM>();
         ...
         public static List<TrackingVM> SelectOne(List<TrackingCmbDTO> trackCmb,string type)
         {
             ...
             try
             {
                 ...
                 ReceiveNo = trackingsCmbDto[0].ReceiveNo;
                 IssueNo = trackingsCmbDto[0].IssueNo;
                 SaleInvoiceNo = trackingsCmbDto[0].SaleInvoiceNo;
                 TransType = type;
                 FormTracking frm = new FormTracking();
                 if (type=="Receive")
                 {
                     frm.rbtnReceive.Checked = true;
                 }
                 else if (type == "issue")
                 {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string VatName = OrdinaryVATDesktop.SanitizeInput(txtVatName.Text);
                string DateText = OrdinaryVATDesktop.SanitizeInput(txtDate.Text);


                //TrackingRawsDt = trackingDal.FindTrackingItems(txtFinishItemNo.Text, txtVatName.Text, txtDate.Text,connVM);
                TrackingRawsDt = trackingDal.FindTrackingItems(FinishItemNo, VatName, DateText, connVM);
                ...
                if (TrackingRawsDt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < TrackingRawsDt.Rows.Count; i++)
                    {
                        ...
                        if (rbtnReceive.Checked)
                        {
                            trackingResults = trackingDal.SearchReceiveTrackItems(rawItemNo, IsReceive, ReceiveNo, TransType,connVM);
                        }
                        else if(rbtnSale.Checked)
                        {
                        ...
                        #region Return
                        else if (TransType=="Receive_Return")
                        {
                            trackingResults = trackingDal.SearchTrackingForReturn("Receive", rawItemNo, ReceiveNo,connVM);
                        }
                        else if (TransType=="Sale_Return")
                        {


                            trackingResults = trackingDal.SearchTrackingForReturn("Sale", rawItemNo, SaleInvoiceNo,connVM);
            ...
            try
            {
                if (rbtnReceive.Checked)
                {
                    if (dgvTracking.Rows.Count > 0)
                    {
                        ...
                        {
                            string isReceive = dgvTracking.Rows[i].Cells["Receive"].Value.ToString();
                            string receiveNo = dgvTracking.Rows[i].Cells["TReceiveNo"].Value.ToString();
                            if (isReceive == "Y" && receiveNo == ReceiveNo)
                            {
                ...
                else if (rbtnSale.Checked)
                {
                    if (dgvTracking.Rows.Count > 0)
                    {
                        for (int i = 0; i < dgvTracking.Rows.Count; i++)
                        {
                            ...
                            string saleInvoiceNo = dgvTracking.Rows[i].Cells["TSaleInvoiceNo"].Value.ToString();
                            if (isSale == "Y" && saleInvoiceNo == SaleInvoiceNo)
                            {
                                dgvTracking["Select", i].Value = true;
                            }
                        }
                ...
                else if (TransType=="Receive_Return")
                {
                    if (dgvTracking.Rows.Count > 0)
                    {
                        for (int i = 0; i < dgvTracking.Rows.Count; i++)
                        {
                            if (dgvTracking.Rows[i].Cells["ReturnReceive"].Value.ToString() == "Y")
                            {
                                dgvTracking["Select", i].Value = true;
                            }
                        }
                ...
                else if (TransType == "Sale_Return")
                {
                    if (dgvTracking.Rows.Count > 0)
                    {
                        for (int i = 0; i < dgvTracking.Rows.Count; i++)
                        {
                            if (dgvTracking.Rows[i].Cells["ReturnSale"].Value.ToString() == "Y")
                            {
                                dgvTracking["Select", i].Value = true;
                            }
                        }
        ...
        private void GridSeleted()
        {
            try
            {
                ...
                for (int i = 0; i < dgvTracking.Rows.Count; i++)
                {
                        ...
                        else if (rbtnReceive.Checked)
                        {
                            if (Convert.ToBoolean(dgvTracking["Select", i].Value) == true)
                            {
                                tracking.IsReceive = "Y";
                                tracking.FinishItemNo = dgvTracking["FinishItemNo", i].Value.ToString();
                                tracking.Quantity = Convert.ToDecimal(dgvTracking["Quantity", i].Value.ToString());
                            }
                            else
                            {
                                tracking.IsReceive = "N";
                            }
                            tracking.IsIssue = dgvTracking["Issue", i].Value.ToString();
                            tracking.IsSale = dgvTracking["Sale", i].Value.ToString();
                            tracking.transactionType = "Receive";
                        }
                        else if (rbtnSale.Checked)
                        {
                            if (Convert.ToBoolean(dgvTracking["Select", i].Value) == true)
