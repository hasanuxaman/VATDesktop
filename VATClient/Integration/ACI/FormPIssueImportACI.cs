...
namespace VATClient
{
    public partial class FormPIssueImportACI : Form
    {
        #region Variables
        ...
        {
            FormPIssueImportACI form = new FormPIssueImportACI();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    //DataTable dt = dal.SelectAll(Program.BranchId.ToString(), null, null, null, null, true);

                    //dtTableResult = importDal.GetIssueACIDbData(invoiceNo, transactionType, FromDate, ToDate, dt);
                    //dgvLoadedTable.DataSource = dtTableResult;

                    progressBar1.Visible = true;
                    bgwIssueLoad.RunWorkerAsync();
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                DataTable issueData = dtTableResult.Copy();
                ...
                //sqlResults = issueDal.SaveTempIssue(issueData, transactionType, Program.CurrentUser, Program.BranchId,
                //    () => { });

                sqlResults = issueDal.SaveIssue_Split(param);


                //UpdateOtherDB(issueData);
            }
            catch (Exception ex)
            {
        ...
        private void UpdateOtherDB(DataTable salesData)
        {
            try
            {
                if (sqlResults[0].ToLower() == "success" && selectedType == CONST_DATABASE)
                {
                    ...
                    string[] results = importDal.UpdateACITransactions(table, settingVM.BranchInfoDT, null, "Issues");

                    if (results[0].ToLower() != "success")
                    {
                        string message = "These Id and Issues failed to insert to other database\n";
            ...
            catch (Exception e)
            {
                ...
                FileLogger.Log("ACI", "UpdateOtherDB", e.Message);
        ...
        private void dgvLoadedTable_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                string date = dgvLoadedTable.CurrentRow.Cells["Issue_DateTime"].Value.ToString();
                string refNo = dgvLoadedTable.CurrentRow.Cells["Reference_No"].Value.ToString();


                _saleRow += "~" + Convert.ToDateTime(date).ToString("ddyy") + "~" + refNo;
        ...
        private void bgwIssueLoad_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                BranchProfileDAL dal = new BranchProfileDAL();
        ...
        private void bgwIssueLoad_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                dgvLoadedTable.DataSource = dtTableResult;
