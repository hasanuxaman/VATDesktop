...
using VATClient.Integration.Nourish;
using VATClient.ReportPreview;
using VATServer.Library.Integration;

namespace VATClient
{
    public partial class FormTransferImportNourish : Form
    {
        #region Variables
        ...
        public string transactionType_Integration;
        ...
        public FormTransferImportNourish()
        {
            InitializeComponent();
        ...
        {
            FormTransferImportNourish form = new FormTransferImportNourish();


            form.preSelectTable = "Transfer";
            form.transactionType = transactionType;

            form.transactionType_Integration = transactionType.ToLower() == "61out" ? "RAW" : "FINISH";
        ...
        private void GetSearchData(string invoiceNo = "", bool withIsProcessed = false)
        {
            ...
            string value = commonDal.settingsDesktop("CompanyCode", "Code");

            param = new IntegrationParam
            {
                TransactionType = transactionType_Integration,
                RefNo = invoiceNo,
                dtConnectionInfo = dt,
                FromDate = FromDate,
                ToDate = ToDate,
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                DataTable data = dtTableResult.Copy();
                ...
                DataTable dt = branchProfileDal.SelectAll(Program.BranchId.ToString(), null, null, null, null, true, connVM);
                ...
                {
                    InvoiceDateTime = Convert.ToDateTime(EntryTime).ToString("yyyy-MM-dd"),
                    InvoiceTime = Convert.ToDateTime(EntryTime).ToString(" HH:mm:ss"),
                    IsEntryDate = true,
                ...
                NourishIntegrationDAL integrationDal = new NourishIntegrationDAL();

                ResultVM resultvm = integrationDal.SaveTransfer(integrationParam, connVM);

                sqlResults[0] = resultvm.Status;
                sqlResults[1] = resultvm.Message;
            }
            catch (Exception ex)
            {
                //MessageBox.Show(ex.Message, this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void btnSaveTemp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsRowSelected())
                {
                    MessageBox.Show("Please select a ID for integration");
                    return;
                }
        ...
        private void dgvLoadedTable_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                CommonDAL commonDal = new CommonDAL();
                string refNo = dgvLoadedTable.CurrentRow.Cells["Id"].Value.ToString();
                string branch_code = dgvLoadedTable.CurrentRow.Cells["BranchCode"].Value.ToString();
                string code = commonDal.settingsDesktop("CompanyCode", "Code", null, connVM);
                ...
                IntegrationParam param = new IntegrationParam
                {
                    ...
                    CompanyCode = code,
                    IDs = new List<string>() { refNo }
                };

                progressBar1.Visible = true;

                bgwDetails.RunWorkerAsync(param);
            }
        ...
        private void bgwSaleLoad_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                dtTableResult = nourishIntegrationDal.GetTransferMaster(param, connVM);
            ...
            catch (Exception exception)
            {
                ...
                sqlResults[0] = "fail";
                sqlResults[1] = exception.ToString();
            }
        }
        ...
        private void bgwDetails_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            BranchProfileDAL branchProfileDal = new BranchProfileDAL();
            DataTable dt =
                branchProfileDal.SelectAll(Program.BranchId.ToString(), null, null, null, null, true, connVM);

            param.dtConnectionInfo = dt;

            NourishIntegrationDAL integrationDal = new NourishIntegrationDAL();

            DataTable dtResult = integrationDal.GetTransferDetails(param);
        ...
        private void bgwDetails_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                dt = dt.DefaultView.ToTable(false, "ProductCode", "ProductName", "Quantity", "CostPrice");

                FormDetails.SelectOne(dt);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
