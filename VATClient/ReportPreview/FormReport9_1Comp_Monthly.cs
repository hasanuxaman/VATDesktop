...
namespace VATClient
{
    public partial class FormReport9_1Comp_Monthly : Form
    {
        ...
        public FormReport9_1Comp_Monthly()
        {
            InitializeComponent();
        ...
        private void NullCheck()
        {
            try
            {
                ...
                {
                    fDate = "";
                    fDate = dtpFromDate.MinDate.ToString("MMyyyy");
                }
                else
                {
                    fDate = dtpFromDate.Value.ToString("MMyyyy");// dtpFromDate.Value.ToString("yyyy-MMM-dd");
                }

                if (dtpToDate.Checked == false)
                {
                    sDate = "";
                    sDate = dtpFromDate.MaxDate.ToString("MMyyyy");

                }
                else
                {
                    sDate = dtpToDate.Value.ToString("MMyyyy");//  dtpToDate.Value.ToString("yyyy-MMM-dd");
                }
            }
            #region Catch
        ...
        private void bgwPreview_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            dtresult = string.Equals(parameterVm.ReportType, "sale", StringComparison.CurrentCultureIgnoreCase)
                ? reportDsdal.GetSale9_1_Comparison_Monthly(parameterVm)
                : reportDsdal.GetPurchase9_1_Comparison_Monthly(parameterVm);
