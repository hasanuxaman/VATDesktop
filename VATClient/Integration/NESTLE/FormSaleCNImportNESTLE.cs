...
namespace VATClient
{
    public partial class FormSaleCNImportNESTLE : Form
    {
        #region Variables
        ...
        {
            FormSaleCNImportNESTLE form = new FormSaleCNImportNESTLE();
        ...
        private void DataLoad()
        {
            ...
            {
                dtTableResult = _IntegrationDAL.GetSource_SaleCNData_Master(paramVM,connVM);

            }
            else
            {
                dtTableResult = _IntegrationDAL.GetSource_SaleDNData_Master(paramVM,connVM);
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if(paramVM.TransactionType.ToLower()=="credit")
                {
                    dtTableResult = _IntegrationDAL.GetSource_SaleCNData_Details(paramVM,connVM);
                }
                else
                {
                    dtTableResult = _IntegrationDAL.GetSource_SaleDNData_Details(paramVM,connVM);
                ...
                TableValidation(SalesData);
                    ...
                    UpdateOtherDB(SalesData,paramVM.TransactionType);
        ...
        {
            FormNestleSaleReturnDetail frm = new FormNestleSaleReturnDetail();
            frm.txtChalanNo.Text = ChalanNo;
            frm.txtTransactionType.Text = transactionType;
            frm.ShowDialog();
        }


        private void UpdateOtherDB(DataTable table,string TransactionType)
        {
            try
            {
                if (sqlResults[0].ToLower() == "success")
                {
                    ...
                    string[] results = importDal.UpdateTransactions(table, settingVM.BranchInfoDT, "SaleInvoiceReturns", TransactionType,connVM);
