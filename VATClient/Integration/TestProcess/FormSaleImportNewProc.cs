...
namespace VATClient
{
    public partial class FormSaleImportNewProc : Form
    {
        #region Variables
        ...
        {
            FormSaleImportNewProc form = new FormSaleImportNewProc();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    //DataTable data = importDal.GetDB2Data();
        ...
        private void GetSearchData()
        {
            ...
            if (chkExcel.Checked)
            {
                dataSet = importDal.GetSaleNewProcData(invoiceNo, null, FromDate, ToDate);
            }
            else
            {
            ...
            IsExcel = chkExcel.Checked;
            if (dataSet != null && dataSet.Tables.Count == 2)
            {
                dtMaster = dataSet.Tables[1];
                dtDetails = dataSet.Tables[0];

                DataColumn dataColumn = new DataColumn()
                    {ColumnName = "Check", DataType = typeof(Boolean), DefaultValue = false};
        ...
        private bool IsRowSelected()
        {
            ...
            DataTable gridData = (DataTable)gridView.DataSource;
            ...
            if (dtTableResult.Columns.Contains("IsProcessed"))
            {
                dtTableResult.Columns.Remove("IsProcessed");
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                {
                    sqlResults[0] = "";
                    MessageBox.Show("Please select at least one row which is not processed");
                    return;
                }
                ...
                BulkCallBack();
                sqlResults = salesDal.SaveAndProcess(salesData, BulkCallBack, Program.BranchId,"",null,null,null,null,Program.CurrentUserID);
        ...
        private void UpdateOtherDB(DataTable salesData)
        {
            if (sqlResults[0].ToLower() == "success" && selectedType == CONST_DATABASE)
            {
                ...
                try
                {
                    ...
                    string[] results = importDal.UpdateSaleTransactions(table, IsExcel,connVM);
        ...
        private void bgwPost_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                ResultVM vm = saleDal.Multiple_SalePost(SaleVM,null,null,connVM);
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string invoices = "'" + string.Join("','", postIdList) + "'";
                ...
                ReportDocument reportDocument = _reportClass.Report_VAT6_3_Completed(invoices, null, false, false, false, false, false, false
                    , true, 0, 0, false, false, false, false,false,false,connVM);
