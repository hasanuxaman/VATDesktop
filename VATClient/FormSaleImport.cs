...
namespace VATClient
{
    public partial class FormSaleImport : Form
    {
        #region Variables
        ...
        {
            FormSaleImport form = new FormSaleImport();
        ...
        private void LoadDataGrid()
        {
            try
            {
                ...
                if (selectedType == CONST_EXCEL)
                {
                    ...
                    {
                        throw new ArgumentNullException("", "No data available or sheet name not match. Sheet name must be 'SaleM'");
                    }
                    if (dtTableResult.Columns.Contains("ImportIDExcel"))
                    {
                        dtTableResult.Columns.Remove("ImportIDExcel");
                    }
                    if (dtTableResult.Columns.Contains("BOMReference"))
                    {
                        dtTableResult.Columns.Remove("BOMReference");
                    }
                    ...
                    bgwSaleExcellvaidation.RunWorkerAsync();
                ...
                else if (selectedType == CONST_XML)
                {
                    ...
                    dtTableResult = GetTableFromXML(CONST_SALETYPE);
        ...
        private DataTable GetTableFromXML(string tableName)
        {
            #region Variables
            DataTable dt = new DataTable();
            string files = Program.ImportFileName;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (chkOldDb.Checked)
                    {
                        ...
                        var invoiceIds = salesData.AsEnumerable().GroupBy(x => x["ID"]).Select(g => g.Key.ToString()).ToList();

                        var dt = salesDal.CheckInvoiceNoExist(invoiceIds,connVM);

                        if (dt.Rows.Count > 0)
                        {
                            throw new Exception("Invoice Ids Already Exists");
                        }
                    }
                    ...
                    if (chkOldDb.Checked)
                    {
                        sqlResults = salesDal.SaveAndProcessOtherDb(salesData, BulkCallBack, Program.BranchId,connVM);
                    }
                    else
                    {
                        ...
                        //saleDataSet.Tables.Add(salesData);

                        //var saleXML = OrdinaryVATDesktop.GetXMLFromDataSet(saleDataSet);


                        //var resultXML = saleAPI.ImportSale(saleXML, BulkCallBack, Program.BranchId, Program.GetAppVersion());

                        //var set = OrdinaryVATDesktop.GetDataSetFromXML(resultXML);

                        //sqlResults[0] = set.Tables[0].Rows[0][0].ToString();
                        ...
                        else if (transactionType.ToLower() == "other" || transactionType.ToLower() == "credit" || transactionType.ToLower() == "debit" || transactionType.ToLower() == "ServiceNS".ToLower() || transactionType.ToLower() == "Export".ToLower())
                        {
                            ...
                            //string saleXML = OrdinaryVATDesktop.GetXMLFromDataSet(saleDataSet);

                            //string resultXML = saleAPI.ImportSale(saleXML, BulkCallBack, Program.BranchId, Program.GetAppVersion());

                            //DataSet set = OrdinaryVATDesktop.GetDataSetFromXML(resultXML);
                            ...
                            //sqlResults = salesDal.ImportExcelIntegrationDataTable(salesData,
                            //    saleMasterVm);

                            //salesData = salesData.DefaultView.ToTable(true, "ID");


                            //salesData.Columns.Add(new DataColumn()
                            //    {ColumnName = "Check", DataType = typeof(bool), DefaultValue = true});
                            #endregion
            ...
            try
            {
               ...
               // ReceiveDAL rDal = new ReceiveDAL();
                ...
                if (loadedTable == CONST_SALETYPE)
                {
                    ...
                    ResultVM TSubTotal = new ResultVM();
                    ResultVM ZSubTotal = new ResultVM(); ;
                    if (dtTableResult.Columns.Contains("SubTotal"))
                    {
                        TSubTotal = oDal.ExcelValidationNumeric(dtTableResult, "SubTotal", "ID");
                        //ZSubTotal = oDal.ExcelValidationZeroCheck(dtTableResult, "SubTotal", "ID");
                    }
