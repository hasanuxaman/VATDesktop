...
namespace VATClient
{
    public partial class FormPurchaseImportBSL  : Form
    {
        #region Variables
        ...
        {
            FormPurchaseImportBSL form = new FormPurchaseImportBSL();

            form.IsCDN = cdn;
            
            form.preSelectTable = "Data";
        ...
        private void typeLoad()
        {
            ...
            code = new CommonDAL().settingValue("CompanyCode", "Code", connVM);

            cmbImportType.SelectedItem = CONST_EXCEL;
            cmbImportType.Text = value;

            cmbSearchField.Items.Add("DocumentDate");
            cmbSearchField.Items.Add("DocumentNumber");
            cmbSearchField.Items.Add("Type");

            cmbSearchField.Text = "DocumentDate";
        ...
        private void LoadDataGrid()
        {
            try
            {
                ...
                if (selectedType == CONST_EXCEL)
                {
                    ...
                    dtTableResult = ds.Tables["Data"];

                    BSLIntegrationDAL dal = new BSLIntegrationDAL();
                    dal.PurchaseTableValidation(dtTableResult);

                    #region Add Column
                    if (!dtTableResult.Columns.Contains("BranchId"))
                    {
                        var BranchId = new DataColumn("BranchId") { DefaultValue = Program.BranchId };
                        dtTableResult.Columns.Add(BranchId);
                    }
                    if (!dtTableResult.Columns.Contains("UserId"))
                    {
                        var UserId = new DataColumn("UserId") { DefaultValue = Program.CurrentUser };
                        dtTableResult.Columns.Add(UserId);
                    }
                    if (!dtTableResult.Columns.Contains("Transection_Type"))
                    {
                        var transection_Type = new DataColumn("Transection_Type") { DefaultValue = transactionType };
                        dtTableResult.Columns.Add(transection_Type);
                    }
                    if (!dtTableResult.Columns.Contains("CompanyCode"))
                    {
                        var companyCode = new DataColumn("CompanyCode") { DefaultValue = code };
                        dtTableResult.Columns.Add(companyCode);
                    }

                    #endregion
                    

                    dtTableResult = OrdinaryVATDesktop.DtDateCheck(dtTableResult, new string[] { "Booking_Date", "Document_Date" });
                    loadedTable = CONST_PURCHASETYPE;
                    this.progressBar1.Visible = true;
                    bgwExcelUploadBSL.RunWorkerAsync();
                }
                #endregion
                #region Text
                else if (selectedType == CONST_TEXT)
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    param.RefNo = invoiceNo;
                    param.FromDate = FromDate;
                    param.ToDate = ToDate;
                    param.TransactionType = transactionType;
                    param.dtConnectionInfo = dt;
                    param.SearchField = cmbSearchField.Text.ToLower();


                    if (param.SearchField == "vendor_name" && param.RefNo.Length < 3)
                    {
                        throw new Exception("At least enter 3 character for search");
                    }
                    ...
                    bgwLoadPurchase.RunWorkerAsync();
        ...
        private void SaveSale()
        {
            ...
            try
            {
                ...
                sqlResults = saleDal.ImportData(dtSaleM, dtSaleD, dtSaleE, CommercialImporter,Program.BranchId,null,Program.CurrentUserID);
            }
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                sqlResults = dal.SavePurchaseData(dtTableResult, null);
        ...
        private void UpdateOtherDB(DataTable table)
        {
            if (sqlResults[0].ToLower() == "success" && selectedType == CONST_DATABASE)
            {
                ...
                string[] results = importDal.UpdateACITransactions(table, settingVM.BranchInfoDT, null, "PurchaseInvoices");
        ...
        private void btnSaveTemp_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(loadedTable))
                {
                ...
                if (!IsRowSelected())
                {
                    MessageBox.Show(this, "Please Atleast Select a Row");
                    return;
                }
        ...
        private void dgvLoadedTable_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (cmbImportType.Text == CONST_DATABASE)
            {
                if (dgvLoadedTable.CurrentRow != null)
        ...
        private void bgwLoadPurchase_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ImportDAL importDal = new ImportDAL();
                dtTableResult = importDal.GetPurchaseACIDbData(param);
        ...
        private void bgwLoadPurchase_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
        ...
        private void bgwExcelUploadBSL_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (loadedTable == CONST_SALETYPE)
                {
                    ...
                    ResultVM TRebateDate = oDal.ExcelValidationDateTime(dtTableResult, "RebateDate", "ID");
                    ResultVM TPost = oDal.ExcelValidationYNCheck(dtTableResult, "Post", "ID");
                    ResultVM TWith_VDS = oDal.ExcelValidationYNCheck(dtTableResult, "With_VDS", "ID");
                    ResultVM TQuantity = oDal.ExcelValidationNumeric(dtTableResult, "Quantity", "ID");
                    ResultVM TTotal_Price = oDal.ExcelValidationNumeric(dtTableResult, "Total_Price", "ID");
                    ResultVM TType = oDal.ExcelValidationPTypeCheck(dtTableResult, "Type", "ID");
                    ResultVM TSD_Amount = oDal.ExcelValidationNumeric(dtTableResult, "SD_Amount", "ID");
                    ResultVM TVAT_Amount = oDal.ExcelValidationNumeric(dtTableResult, "VAT_Amount", "ID");
                    ResultVM TVDSAmount = oDal.ExcelValidationNumeric(dtTableResult, "VDSAmount", "ID");
                    ResultVM TCnF_Amount = oDal.ExcelValidationNumeric(dtTableResult, "CnF_Amount", "ID");
                    ResultVM TInsurance_Amount = oDal.ExcelValidationNumeric(dtTableResult, "Insurance_Amount", "ID");
                    ResultVM TAssessable_Value = oDal.ExcelValidationNumeric(dtTableResult, "Assessable_Value", "ID");
                    ResultVM TOthers_Amount = oDal.ExcelValidationNumeric(dtTableResult, "Others_Amount", "ID");
                    ResultVM TIsRebate = oDal.ExcelValidationYNCheck(dtTableResult, "IsRebate", "ID");
                   
                    errormessage.AddRange(TInvoice_Date.ErrorList);
                    errormessage.AddRange(TReceive_Date.ErrorList);
                    errormessage.AddRange(TRebateDate.ErrorList);
                    errormessage.AddRange(TPost.ErrorList);
                    errormessage.AddRange(TWith_VDS.ErrorList);
                    errormessage.AddRange(TQuantity.ErrorList);
                    errormessage.AddRange(TTotal_Price.ErrorList);
                    errormessage.AddRange(TType.ErrorList);
                    errormessage.AddRange(TSD_Amount.ErrorList);
                    errormessage.AddRange(TVAT_Amount.ErrorList);
                    errormessage.AddRange(TVDSAmount.ErrorList);
                    errormessage.AddRange(TCnF_Amount.ErrorList);
                    errormessage.AddRange(TInsurance_Amount.ErrorList);
                    errormessage.AddRange(TAssessable_Value.ErrorList);
                    errormessage.AddRange(TOthers_Amount.ErrorList);
                    errormessage.AddRange(TIsRebate.ErrorList);                   

                }
            }
            catch (Exception ex)
            {
                FileLogger.Log(this.Name, "bgwExcelUploadBSL_DoWork", ex.ToString());
                throw;
            }
        ...
        private void bgwExcelUploadBSL_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
            ...
            {
                FileLogger.Log(this.Name, "bgwExcelUploadBSL_RunWorkerCompleted", ex.ToString());
                MessageBox.Show(ex.Message, this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            finally
            {
        ...
        private void btnAutoLoad_Click(object sender, EventArgs e)
        {
            ...
            string baseDirectory = "D:\\BSL";
            string copyDirectory = "D:\\CopyBSL";
            ...
            try
            {
                ...
                foreach (string filePath in files)
                {
                    ...
                    string destinationPath = Path.Combine(copyDirectory, fileName);
                ...
                BSLIntegrationDAL dal = new BSLIntegrationDAL();
                ImportVM paramVm = new ImportVM();
                paramVm.FilePath = fileDirectory;
                ds = new ImportDAL().GetDataSetFromExcel(paramVm);
                dt = ds.Tables["Data"];
                dal.PurchaseTableValidation(dt);
                retResults = dal.SavePurchaseData(dt, null);
                ...
                {
                    MessageBox.Show(this, retResults[4], this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    MessageBox.Show(this, "Data Save Successfully.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
