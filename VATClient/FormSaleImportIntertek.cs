...
namespace VATClient
{
    public partial class FormSaleImportIntertek : Form
    {
        #region Variables
        ...
        {
            FormSaleImportIntertek form = new FormSaleImportIntertek();
        ...
        private void BrowsFile()
        {
            ...
            try
            {
                OpenFileDialog fdlg = new OpenFileDialog(); //CSV files (*.csv*)|*.csv*|
        ...
        {
            if (InvokeRequired)
                Invoke((MethodInvoker)delegate { PercentBar(step); });
        }
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                //sqlResults = saleDal.SaveAndProcessITS(salesData, () => { }, Program.BranchId, Program.GetAppVersion(),
                //    transactionType, Program.CurrentUser);
                ...
                sqlResults = saleDal.SaveSaleITS_Split(param,"",connVM);
        ...
        private void btnSaveTemp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (
                    MessageBox.Show("Are you sure to create invoice with this date: " + dtpSaleTranDate.Value.ToString("dd-MMM-yyyy HH:mm:ss"), this.Text,
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question) != DialogResult.Yes)
                {
                    return;
                }
                string[] delColumn= new string[] {"Transaction Date"};
                OrdinaryVATDesktop.DtDeleteColumns(dtTableResult, delColumn);
                OrdinaryVATDesktop.DtColumnAdd(dtTableResult, "Transaction Date", dtpSaleTranDate.Value.ToString("yyyy-MM-dd HH:mm:ss"), "string");
                //foreach (DataRow row in dtTableResult.Rows)
                //{
                //    row["TransactionDate"] = dtpSaleTranDate.Value.ToString("dd-MMM-yyyy");
                //}
