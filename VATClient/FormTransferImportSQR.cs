...
namespace VATClient
{
    public partial class FormTransferImportSQR : Form
    {
        #region Variables
        ...
        public FormTransferImportSQR()
        {
            InitializeComponent();
            connVM = Program.OrdinaryLoad();
        }

        public static string SelectOne(string transactionType, bool cdn = false)
        {
            FormTransferImportSQR form = new FormTransferImportSQR();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (branchCode != "UEN")
                    {
                        if (dt.Rows[0]["DbType"].ToString().ToLower() == "sql")
                        {
                            dtTableResult = dal.GetFGSQRDbData_New(ids, dt, txtId.Text);
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                dtTableResult.Columns.Add(new DataColumn() {ColumnName = "SDRate", DefaultValue = 0});
                ...
                DataTable salesData = filterdData;
                ...
                transferData.Columns.Add(new DataColumn()
                    {ColumnName = "UserId", DefaultValue = Program.CurrentUserID});

                sqlResults = transferIssueDAL.SaveTempTransfer_Integration(transferData, Program.BranchCode, transactionType,
                    Program.CurrentUserID, Program.BranchId, () => { });
