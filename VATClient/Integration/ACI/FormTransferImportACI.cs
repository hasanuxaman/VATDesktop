...
namespace VATClient
{
    public partial class FormTransferImportACI : Form
    {
        #region Variables
        ...
        {
            FormTransferImportACI form = new FormTransferImportACI();
        ...
        void NullCheck()
        {
            ...
            EntryTime = dptTime.Checked ? dptTime.Value.ToString("HH:mm:ss tt") : "";
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    dt = dal.SelectAll(Program.BranchId.ToString(), null, null, null, null, true,connVM);


                    progressBar1.Visible = true;

                    bgwLoadTransfer.RunWorkerAsync();
        ...
        private void bgwBigData_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                sqlResults = transferIssueDAL.SaveTempTransfer(transferData, Program.BranchCode, transactionType,
                    Program.CurrentUser, Program.BranchId, () => { }, null, null, false,connVM, EntryTime,Program.CurrentUserID);
                //UpdateOtherDB(transferData);
        ...
        private void UpdateOtherDB(DataTable table)
        {
            if (sqlResults[0].ToLower() == "success" && selectedType == CONST_DATABASE)
            {
                ...
                string[] results = importDal.UpdateACITransactions(table, settingVM.BranchInfoDT,connVM);
        ...
        private void bgwLoadTransfer_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ImportDAL importDal = new ImportDAL();

                dtTableResult = importDal.GetTransferACIDbData(invoiceNo, transactionType, FromDate, ToDate, dt,connVM);

            }
            catch (Exception exception)
            {
                FileLogger.Log("TransferIssueACI","transfer",exception.Message);
            }
        }

        private void bgwLoadTransfer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                dgvLoadedTable.DataSource = dtTableResult;
